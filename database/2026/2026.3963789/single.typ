...
#make_title(toml("info.toml"), title_override: none, abstract_content: [
...
#[
  #show: mode_2col
    ...
    + We introduce #strong[Benchmark Reduction] ($lt.eq_"bench"$) and show that any statable problem can be reduced, in polynomial time, to "constructing a benchmark of that problem and fine-tuning a large model";
    ...
    #figure(
      image("assets/figure1_scaling_law_en.png", alt: "Figure 1"),
    ...
    Our method generalizes seamlessly to the remaining Millennium Prize Problems. These follow-up benchmarks all share the $lt.eq_"bench"$ framework; they are not independent theoretical frameworks but downstream applications of Benchmark Reduction, each requiring only a problem-specific reduction template:
  ...
  + #strong[Section 2:] added Remark 1 (the client-time model), including the breach-of-contract exclusion clause, $lt.eq_p ⊆ lt.eq_"bench"$, and the equivalence of the reverse inclusion with $P = "NP"$; the former Remark 1 is renumbered as Remark 2 (addresses W1 / Q1);
  ...
  + #strong[Section 5.2:] added problem-specific reduction templates for the follow-up benchmarks and clarified that they are downstream applications of $lt.eq_"bench"$ rather than independent frameworks (addresses M2);

  #place(top + left, dx: 999mm)[
    @cook1971complexity
...
#review_file_page[
  #strong[稿件题目：]Proof by Benchmark: P = NP with 87.3% Confidence
